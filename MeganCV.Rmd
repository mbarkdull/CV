...
inReview <- Publications %>%
  filter(Type == "inReview") %>%
  pull("Details") 
printNumberedList(inReview)
```
\normalsize 

**Submitted for presubmission inquiry:** 

\scriptsize 
Manucripts available upon request. 

```{r results='asis', cache=FALSE}
presubmission <- Publications %>%
  filter(Type == "presubmission") %>%
  pull("Details") 
printNumberedList(presubmission)
```
\normalsize 
...
<!-- \scriptsize -->
```{r results='asis', cache=FALSE}
#inPress <- Publications %>%
  #filter(Type == "inPress") %>%
  #pull("Details") 
#printNumberedList(inPress)
```
<!-- \normalsize -->
...
**Research grants     (Total amount: $14,000)**
\scriptsize
```{r}
Awards %>%
